...
class ChatBot:
  ...
  def get_response_regex(self, prompt: str, regex: str) -> Generator[str, None, None]:
    """
    ...
    """
    buffer = ""
    for token in self.get_response(prompt):
      buffer += token
  ...
  def get_response_by_punctuation(self, prompt: str) -> Generator[str, None, None]:
    """
    sends the response of a prompt in sentences instead of tokens. 
    Look at send_regex() for more detail & control.
    """
    return self.get_response_regex(prompt, r'(?<=[.!?])\s+')
    
if __name__ == "__main__":
  chat = ChatBot().set_model("gemma3:4b")

  for item in chat.get_response_by_punctuation("Hello, world! How do I write a REST API in python?"):
    print(item)
