...
def configure_chat_settings(
    *,
    ...
    if max_history_messages is not None:
        if int(max_history_messages) < 0:
            raise ValueError("Max history messages must be non-negative.")
        MAX_HISTORY_MESSAGES = int(max_history_messages)
    if max_history_chars is not None:
        if int(max_history_chars) < 0:
            raise ValueError("Max history chars must be non-negative.")
        MAX_HISTORY_CHARS = int(max_history_chars)
    if external_api_timeout is not None:
        if float(external_api_timeout) <= 0:
            raise ValueError("External API timeout must be > 0.")
        EXTERNAL_API_TIMEOUT = float(external_api_timeout)


def set_chat_limits(max_tokens: int, max_history_messages: int, max_history_chars: int) -> None:
...
def get_full_response(prompt: str) -> str:
    ...
    if is_ollama_provider():
        ...
        _log_if_completion_truncated(finish_reason=_extract_ollama_finish_reason(response_obj))
    else:
        payload = {
            "model": current_model,
            "messages": messages,
